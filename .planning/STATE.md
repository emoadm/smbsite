...
| 260511-1go | TRIAGE Batch A — close 4 Phase 2 contract-refresh failures: drop dup `aria-label` from Footer brand link (#4), replace Phase 1 redirect assertion with Header "Вход" link visibility check in login.spec (#6), add source-dropdown picker (`qr_letter` / "QR код в писмо") to registration.spec before submit (#8), scope email-leak regex from `page.content()` to `<main>` in proposals-public.spec (#7). 1 code-bug + 3 test-bugs. Expected CI Playwright 42/12 → 46/8. CI human-verify pending. | 2026-05-10 | b4536b3 | [260511-1go-batch-a-phase-2-contract-refresh](./quick/260511-1go-batch-a-phase-2-contract-refresh/) |
