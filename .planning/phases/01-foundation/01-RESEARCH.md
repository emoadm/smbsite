...
|---|----------|-----------|----------------|
| Q1 | **Next.js version: pin at 15.3.x or upgrade to 16.2.4?** — Registry shows 16.2.4 as current stable. CLAUDE.md specifies "15.x". Payload 3.73+ supports Next.js 16. Turbopack works better on 16. | Yes — determines Dockerfile, Turbopack mode, next-intl middleware naming | Pin at **15.3.x** for maximum stability certainty during Phase 1. Upgrade to 16.x in a later phase. **RESOLVED → plan 1.01** (`package.json` pins `next@15.3.5`). |
| Q2 | **Coalition's sender domain** — must be confirmed by end of Phase 1 plan-week 1 (D-17). Until then, DNS warm-up tasks cannot proceed. What is the placeholder domain for development? | Yes for NOTIF-07 | Use a dev domain (e.g., `dev.smb-balgaria.bg`) for DNS setup and Brevo authentication; replace with production domain before QR campaign. Planner must include "domain confirmed" as a blocking deliverable in wave 1. **RESOLVED → plan 1.13 Task 1.13.1** (blocking checkpoint: coalition confirms sender domain; downstream warm-up tasks gated). |
| Q3 | **Coalition logo SVG** — must be obtained from coalition (D-17 branding note, BRAND-02). Is the SVG available at sinyabulgaria.bg `/media/2023/08/logo-vector-1.svg`? | No (placeholder acceptable for Phase 1 dev) | Planner includes "logo asset received and committed" as a deliverable. Use colored placeholder rectangle until received. **RESOLVED → plan 1.04 Task 1.04.4** (`public/logo-placeholder.svg`, navy `#004A79`, swap to real SVG when delivered). |
| Q4 | **Brevo account and API key** — must be created before Phase 1 can send any email. Is the Brevo account under the coalition's email domain? | Yes for NOTIF-07 start | Planner includes "Brevo account created + API key provisioned" in wave 0 task list. **RESOLVED → plan 1.13 user_setup#brevo** (`BREVO_API_KEY` source documented; sender authentication checklist in 01-DELIVERABILITY-CHECKLIST.md Section B). |
| Q5 | **Payload CMS admin first-user setup** — D-25 says "gated to localhost-only on first deploy." How is the initial Payload admin created in a Fly.io container without localhost? | Yes | Create admin via `fly ssh console` and `node` REPL against the running container, or use Payload's `create-first-user` route behind a one-time feature flag. Planner must define the exact procedure. **RESOLVED → plan 1.12 Task 1.12.3** (OPS-RUNBOOK § 2 documents the exact `fly ssh console` + `payload.create` invocation). |
| Q6 | **Better Stack EU vs Axiom EU** — Claude's Discretion (D-21). Planner picks. Research recommends Better Stack EU for all-in-one posture. | No | Planner confirms Better Stack EU or overrides with Axiom EU. **RESOLVED → plan 1.11** (Better Stack EU selected; `@logtail/pino` transport + `BETTERSTACK_SOURCE_TOKEN` env var). |
| Q7 | **OTP token hashing in Auth.js adapter** — the default `@auth/drizzle-adapter` stores tokens plaintext. Hardening to HMAC-SHA256 requires a custom adapter override. Is this in scope for Phase 1 or deferred? | No (security best practice, not hard requirement) | Recommend including in Phase 1 as part of AUTH-05 security hardening. The 6-digit OTP is low-entropy — plaintext storage combined with DB access = trivial OTP bypass. **RESOLVED → plan 1.05 Task 1.05.1 + 1.05.2** (HMAC-SHA256 hashing in `src/lib/auth-utils.ts`; `persistHashedOtp` helper in `src/lib/auth.ts` ensures plaintext never reaches the DB). |
