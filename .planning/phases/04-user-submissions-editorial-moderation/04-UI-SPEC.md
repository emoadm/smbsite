...
- **Действие:** `<Button variant="outline" size="sm">` "Прегледай" — opens the review drawer/dialog.
...
**Review Panel (opens on "Прегледай" click):**
...
Simple 2-button confirm: Heading "Одобри предложението?" Body: "Предложението ще стане видимо публично на страница /предложения." Actions: `<Button variant="outline">` "Отказ" + `<Button variant="default">` "Одобри".

**Reject confirmation `<Dialog>` (nested):**

Heading "Отхвърли предложението?" Body: "Членът ще види статус 'Отхвърлено' и бележката ти." Validates that `moderatorNote` is non-empty before enabling the confirm button — server-side Zod also enforces this. Actions: `<Button variant="outline">` "Отказ" + `<Button variant="destructive">` "Отхвърли".
...
Heading: "Спри акаунта на {full_name}?" Body: "Посочи причина — тя ще бъде записана в журнала на модерацията." `<Textarea rows={2} required>`. Actions: `<Button variant="outline">` "Отказ" + `<Button variant="destructive">` "Спри акаунта". Reason is mandatory (server Zod enforces `minLength(10)`).
...
| Admin approve confirm | "Одобри предложението?" | `admin.moderation.approveHeading` |
| Admin approve body | "Предложението ще стане видимо публично на страница /предложения." | `admin.moderation.approveBody` |
| Admin reject confirm | "Отхвърли предложението?" | `admin.moderation.rejectHeading` |
| Admin reject body | "Членът ще види статус 'Отхвърлено' и бележката ти." | `admin.moderation.rejectBody` |
| Admin suspend confirm | "Спри акаунта на {name}?" | `admin.moderation.suspendHeading` |
...
| `ConfirmActionDialog.tsx` | Reusable nested confirm; used for approve / reject / suspend |
...
| Typography scale | Phase 2 UI-SPEC §3.3 + `globals.css` `@theme` |
